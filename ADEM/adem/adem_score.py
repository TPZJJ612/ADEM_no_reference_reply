...
def compute_adem_score(context, model_response, reference_response,
    ...
    with tf.variable_scope('score'):
        ...
        tmp_score = tf.divide((tf.reduce_sum((context * tf.matmul(model_response, M)), axis=1) +
                     tf.reduce_sum((reference_response * tf.matmul(model_response, N)), axis=1) -
                     alpha), beta, name= "model_score1")
        # score.assign(tmp_score)

    return tmp_score, M, N
...
def tf_dynamic_adem_score(context, model_response, reference_response, shape_info):
    ...
    score, M, N = compute_adem_score(context
                                     , model_response
                                     , reference_response
