...
class ADEM(object):
    ...
    ### build graph ###
        self.context_place\
            , self.model_response_place\
            , self.reference_response_place\
            , self.human_score_place\
            , self.new_lr_place\
            , self.train_op\
            , self.loss\
            , self.model_score\
            , self.lr_update = self.build_graph()
    ...
    def predict_on_single_batch( self
                                , predict_session
                                , context
                                , model_response
                                , reference_response
                                ):
        assert(context.shape[0] == model_response.shape[0] == reference_response.shape[0])
        assert(context.shape[1] == self.context_dim)
        assert(model_response.shape[1] == self.model_response_dim)
        assert(reference_response.shape[1] == self.reference_response_dim)

        prediction, loss_val, _ = predict_session.run( [self.model_score, self.loss]
                                                      , feed_dict = {self.context_place : context
                                                      , self.model_response_place : model_response
                                                      , self.reference_response_place : reference_response})
        return prediction, loss_val
    ...
    def dev_step( self
        ...
        prediction, loss_val, _ = predict_session.run( [self.model_score, self.loss, self.train_op]
                                                      , feed_dict = {self.context_place : context
                                                      , self.model_response_place : model_response
                                                      , self.reference_response_place : reference_response
                                                      , self.human_score_place: human_score
